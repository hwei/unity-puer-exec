...
在推送远端之前，可以先用本地 helper 准备 source release：
...
- 检查工作树是否干净，并确认 `v<version>` 没有在本地或 `origin` 上被占用
- 改写 `packages/com.txcombo.unity-puer-exec/package.json`
- 运行默认的 mocked/unit release 测试集
- 按需运行 real-host validation、创建本地 release commit、创建本地 source tag

边界说明：

- 这个 helper 不会推送 commit 或 tag
- 现有 GitHub Actions release pipeline 仍然只会在维护者手动 push source commit 和 `v<version>` tag 后触发
- `--dry-run` 是纯预览模式，不会改文件，也不会跑测试
