...
1. The validation host should be checked out to a clean baseline that does not already contain the formal `unity-puer-exec` package as committed host source.
...
- Concrete host-baseline and manifest-injection details should be distilled separately when they become stable.
