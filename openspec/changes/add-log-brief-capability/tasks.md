...
- [x] 3.5 Verify that `brief_sequence` grows consistently across successive `wait-for-exec` calls when `log_range.start` is held constant
...
- [x] 4.3 Run a help-only Prompt B rerun and record whether `Editor.log` fallback after `unity_stalled` or compile phases decreases
- [x] 4.4 If fallback decreases, archive `improve-wait-for-log-pattern-stall-guidance` as superseded; otherwise record findings for that change to continue

### 4.3 Findings (2026-03-25, haiku subagent, dry-run probe)

- Editor.log fallback **decreased**: haiku agent chose `brief_sequence` + `get-log-briefs` for all observation, explicitly stated no raw Editor.log needed.
- Agent correctly discovered `get-log-briefs --range` pattern from help and used `--refresh-before-exec` for post-C#-write recovery.
- Residual issue: agent assumed `ctx.getProjectRoot()` — ctx contract not visible in `exec --help` Quick Start. Filed as `surface-ctx-contract-in-exec-help` change.

### 4.4 Disposition

- `improve-wait-for-log-pattern-stall-guidance` **retained** (not archived): Prompt B fallback reduced via log-brief, but `wait-for-log-pattern` stall recovery remains an independent gap for non-Prompt-B scenarios.
