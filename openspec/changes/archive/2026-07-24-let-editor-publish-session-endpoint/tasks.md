...
- [x] 8.7 Run the full real-host suite and record the result. **2026-07-24 re-run after `cli-launch-arg-passthrough`**: `UNITY_PUER_EXEC_RUN_REAL_HOST_TESTS=1` with `UNITY_PUER_EXEC_UNITY_LAUNCH_ARGS=["-force-gles30"]` → Ran 19, OK, skipped=3 (batch exclusive-lock skipped while 4 unrelated Editors open; preferred-port rollover ×2 skipped because 55231 held by an unrelated service). Auto-launch of the host succeeded; the private-log case required a one-line tier rename from removed `session_artifact` to `published_endpoint`.
