...
The formal CLI SHALL use `unity-puer-exec` as its single primary entry. The authoritative flat command tree SHALL include `wait-until-ready`, `wait-for-log-pattern`, `wait-for-result-marker`, `get-log-source`, `exec`, and `ensure-stopped`. The formal CLI SHALL NOT include `get-result`.

#### Scenario: Agent discovers the CLI surface after async redesign

- **WHEN** repository docs or help describe the CLI after the redesign is implemented
- **THEN** `unity-puer-exec` is presented as the primary entry
- **AND** long-running work is described without a separate `get-result` continuation command
- **AND** `wait-for-result-marker` is discoverable as the recommended high-level long-running workflow
